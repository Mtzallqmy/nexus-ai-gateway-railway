...
data-labels default false; when true, if a series has labels that text is shown, otherwise the raw values
...
Child tag `<series name? values color? labels? type? axis?>`:

- `values` numeric array, length must = category count; accepts pure numbers only (not `"45%"`, `"120M"`)
- `labels` display-text array, same length as values, shown in place of values when data-labels is on (`"120M"`, `"45%"`, whatever you want); **no commas in the text**, abbreviate large numbers (K/M/B)
...
<!-- Pie chart: labels explicitly write the share -->
<chart type="pie" x="720" y="160" width="400" height="360" font="body" categories="East,South,North" color="brand,#5B9BD5,#A5A5A5" data-labels="true">
  <series values="45,30,25" labels="45%,30%,25%"/>
</chart>
...
4. A chart series gets no color; a pie/donut with multiple series; `values`/`labels` length ≠ category count; a series `type` outside column/line/area, or a series has `type` while the chart-level type is not one of those three
